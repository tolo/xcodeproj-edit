...
class PerformanceProfiler {
  ...
  func measureBatchOperation<T>(_ operationName: String, items: [T], operation: (T) throws -> Void) rethrows {
    ...
    for (index, item) in items.enumerated() {
      if isVerbose && items.count > 10 && index % (items.count / 10) == 0 {
        print("  Progress: \(index)/\(items.count) (\(Int(Double(index) / Double(items.count) * 100))%)")
      }
      try operation(item)
    }
  }
  ...
  func benchmarkOperation(_ name: String, iterations: Int = 100, operation: () throws -> Void) rethrows {
