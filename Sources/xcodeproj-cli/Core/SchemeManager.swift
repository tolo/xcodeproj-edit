...
class SchemeManager {
    ...
    func createScheme(
        ...
        let scheme = XCScheme(
            name: name,
            lastUpgradeVersion: xcodeproj.lastKnownUpgradeCheck,
            version: "1.7",
    ...
    func setSchemeConfiguration(
        ...
        var scheme = try loadScheme(name: schemeName)
        ...
        if let config = testConfig, var testAction = scheme.testAction {
            testAction.buildConfiguration = config
            scheme.testAction = testAction
        }
        
        if let config = profileConfig, var profileAction = scheme.profileAction {
            profileAction.buildConfiguration = config
            scheme.profileAction = profileAction
        }
        
        if let config = analyzeConfig, var analyzeAction = scheme.analyzeAction {
            analyzeAction.buildConfiguration = config
            scheme.analyzeAction = analyzeAction
        }
        
        if let config = archiveConfig, var archiveAction = scheme.archiveAction {
            archiveAction.buildConfiguration = config
            scheme.archiveAction = archiveAction
        }
    ...
    func addTargetToScheme(
        schemeName: String,
        targetName: String,
        buildFor: [XCScheme.BuildAction.Entry.BuildFor] = [.running, .testing, .profiling, .archiving, .analyzing]
    ) throws {
        var scheme = try loadScheme(name: schemeName)
        ...
        if var buildAction = scheme.buildAction {
            ...
            buildAction.buildActionEntries.append(entry)
    ...
    func enableTestCoverage(
        ...
        guard var testAction = scheme.testAction else {
            throw ProjectError.operationFailed("Scheme '\(schemeName)' has no test action")
        }
        
        // Enable code coverage
        testAction.codeCoverageEnabled = true
        ...
        if let targetNames = targets {
            ...
            testAction.codeCoverageTargets = coverageTargets
        }
    ...
    func setTestParallelization(
        ...
        let testPlans = enabled ? 
            XCScheme.TestAction.TestPlans(parallelizable: true, randomExecutionOrdering: false) :
            nil
