...
class XcodeProjService {
  ...
  private func _addTarget(name: String, productType: String, bundleId: String, platform: String)
    throws
  {
    ...
    throws
  {
    for targetName in targets {
      ...
      if let configName = configuration {
        ...
        if configs.isEmpty {
          throw ProjectError.operationFailed(
            "Configuration '\(configName)' not found for target: \(targetName)")
        }
      } else {
  ...
  func getBuildSettings(for targetName: String, configuration: String? = nil) throws -> [String:
    [String: Any]]
  {
    let target = try getTarget(targetName)
