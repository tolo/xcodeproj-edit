...
class CrossProjectManager {
    ...
    func addCrossProjectDependency(
        ...
        let containerProxy = PBXContainerItemProxy(
            ...
            remoteGlobalID: externalTargetGUID,
        ...
        print("✅ Added cross-project dependency: \(targetName) -> \(externalTargetName) in \(externalProjectPath)")
    }
    ...
    func removeCrossProjectDependency(
        ...
        if !found {
            throw ProjectError.operationFailed("Dependency '\(externalTargetName)' not found in target '\(targetName)'")
        }
