...
class WorkspaceManager {
        ...
        workspace.data.children.removeAll { child in
            if case let .file(ref) = child {
                if case let .group(path) = ref.location {
                    ...
                    if path == projectPath || 
                       path.hasSuffix(projectPath) || 
                       projectPath.hasSuffix(path) {
                        found = true
                        return true
                    }
                }
    ...
    func addCrossProjectDependency(
        ...
        let projectName = Path(externalProjectPath).lastComponent
        guard let projectRef = pbxproj.fileReferences.first(where: { 
            $0.path == externalProjectPath || $0.name == projectName 
        }) else {
            throw ProjectError.operationFailed("External project '\(projectName)' not found. Add project reference first.")
        }
        ...
        print("✅ Added cross-project dependency: \(targetName) -> \(externalProjectPath):\(externalTargetName)")
    }
